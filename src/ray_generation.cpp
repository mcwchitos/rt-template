...
Payload RayGenerationApp::Miss(const Ray& ray) const
{
	...
	// payload.color = { ray.direction.x, t, 0 };
...
{
	byte3 byteColor{ static_cast<uint8_t>(255.0f * color.x), static_cast<uint8_t>(255.0f * color.y), static_cast<uint8_t>(255.0f * color.z) };
